...
dmtracedump.exe -o *.trace | tr -d '\000' | egrep 'ent.*UncompressedListener\.handleResult \(Ljava\/l' | sort > handleResult-timestamps.txt
...
for i in $( cat handleResult-timestamps.txt | egrep -o 'ent.*UncompressedListener\.handleResult \(Ljava\/l' | tr -d ' ent' | cut -d '.' -f 1 ); do
...
dmtracedump.exe -o *.trace | tr -d '\000' | egrep 'xit.*CloudClient\.addFrames' | sort > addFrames-timestamps.txt
...
for j in $( cat addFrames-timestamps.txt | egrep -o 'xit.*CloudClient\.addFrames' | tr -d ' xit' | cut -d '.' -f 1 ); do
